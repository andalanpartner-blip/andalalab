...
import { BriefIntelligence } from "../BriefIntelligence";
import { DesignDirectionPanel } from "../DesignDirectionPanel";
import { ConceptCompare } from "../ConceptCompare";
import { ConceptStrip } from "../ConceptStrip";
import { RecipeStageSummary } from "../RecipeStageSummary";
...
import { AIStatus, type AIStatusStep } from "./AIStatus";
...
async function postBrief(input: PendingRequest & { projectId: string }): Promise<BriefPipelineResult> {
...
/** Record an immutable artifact-metadata entry against the project (hashes only). */
async function recordArtifact(
  projectId: string,
  entry: { kind: string; hash: string; parent_hash?: string | null; summary?: Record<string, unknown> }
): Promise<void> {
  try {
    ...
    /* best-effort lineage; the design flow does not depend on it */
  }
}
...
export function Workspace({ projectId }: { projectId: string }) {
  ...
  const submitBrief = useCallback(
    async (input: PendingRequest) => {
      ...
      const data = await postBrief({ ...input, projectId });
    ...
    [goToStage, clearGeneratedVisual, projectId]
  );
  ...
  const handleBuildRecipe = useCallback(async () => {
    ...
    if (data.status === "OK") {
      ...
      clearGeneratedVisual();
      void recordArtifact(projectId, { kind: "recipe", hash: data.recipe.recipe_hash, summary: { movement: data.recipe.movement.id } });
      void recordArtifact(projectId, {
        kind: "blueprint",
        hash: data.blueprint.blueprint_hash,
        parent_hash: data.recipe.recipe_hash,
        summary: { zones: data.blueprint.zones.length }
      });
    } else {
